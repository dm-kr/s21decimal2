    ...
    *result = Decimal(0);
    ...
    if (sign1 == 0 && sign2 == 0) {
      ...
      if (s21_is_greater_or_equal(value_1, value_2)) {
        ...
        code = s21_sub_handle(decimal_abs(value_2), decimal_abs(value_1), &res);
      } else {
        // Если уменьшаемое меньше вычитаемомого
        // Вычитаем из уменьшаемого вычитаемое (используя abs), у результата
        // меняем знак v1 = -2; v2 = -1; v1 - v2 = -2 - -1 = -2 + 1 = 1 - 2 = -
        // (2 - 1) = - (abs(v1) - abs(v2))
        code = s21_sub_handle(decimal_abs(value_1), decimal_abs(value_2), &res);
        s21_negate(res, &res);
      }
    }
...
int s21_sub_handle(s21_decimal value_1, s21_decimal value_2,
  ...
  big_decimal res = bd_bin_sub(value_1l, value_2l);
