...
// Узнаем на сколько нужно сдвинуть значение биг децимала чтобы оно поместилось
// в обычный децимал
...
// Находим максимальный инт из двух
...
// Вычисляем дробную часть числа при делении
...
// Приводим два децимала к равному скейлу, записывая их в биг децималы
...
// Находим значение экспоненты из строки, полученной из sprintf
...
// Переводим флоат в виде строки в децимал
...
// Очищаем сервисные биты
void clear_service(s21_decimal *value) { value->bits[3] = 0; }

// Конструктор децимала на основе инта
...
// Проверяем децимал на корректность (Скейл не может быть < 0 или > 28)
// Также два сектора в сервисном инте децимала должны быть свободны
...
// Получаем первый пустой участок сервисного инта
...
// Получаем второй пустой участок сервисного инта
...
// Проверяем децимал на четность
int is_even(s21_decimal value) { return (value.bits[0] & 1) != 1; }

// Отбрасываем нули с конца дробной части децимала
