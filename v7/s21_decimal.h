...
// Отсюда берем минимальное и максимальное значение int
#include <limits.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
// Биг децимал, просто два обычных децимала, скомбинированных друг с другом
typedef struct big_decimal {
  s21_decimal decimals[2];
} big_decimal;

// Общее число бит в децимале
#define ALL_BITS 128
// Размер одного децимала
#define INT_SIZE 32
// Размер части децимала, содержащей само число, без доп. инфы
#define BASE_SIZE 96

// Максимально возможное значение float, которое можно конвертировать в децимал
#define MAX_FLOAT 79228157791897854723898736640.0f
// Минимальное значение
#define MIN_FLOAT \
  0.00000000000000000000000000010000000031710768509710513471352647538147514756461109f

// Массив со степенями десятки от 0 до 38, для ускорения вычислений
...
// Объединение для упрощения доступа к отдельным частям сервисного инта в
// децимале
// В объединении под все его составляющие выделяется одна общая область
// памяти
// В нашем случае выделяем 32 бита
// Bits занимает все 32 бита, структура parts тоже занимает 32 бита, но разбита
// на части
// Каждая занимает соответственно указанному около неё значению
// С помощью двоеточия можно указать сколько конкретно бит будет занимать
// переменная, например "scale : 8" означает, что scale будет занимать 8 бит
